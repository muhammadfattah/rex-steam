...
@section('content')
    <div class="bg-gray p-5">
        ...
        <form action="/manage-game/{{ $game->id }}" method="POST" enctype="multipart/form-data" class="tombol-simpan">
            @csrf
            @method('PUT')
            <div class="mb-2">
                ...
                <input type="text" class="form-control form-control-sm @error('name') is-invalid @enderror" id="name"
                    aria-describedby="name_help" name="name" value="{{ old('name') ?? $game->name }}">
                @error('name')
            ...
            <div class="mb-2">
                ...
                <textarea name="description" id="description" rows="2"
                    ...
                    ria-describedby="description_help">{{ old('description') ?? $game->description }}</textarea>
            ...
            <div class="mb-2">
                ...
                <textarea name="long_description" id="long_description" rows="5"
                    ...
                    ria-describedby="long_description_help">{{ old('long_description') ?? $game->long_description }}</textarea>
            ...
            <div class="mb-2">
                ...
                <select class="form-select form-select-sm @error('category') is-invalid @enderror" id="category"
                    ...
                    @foreach ($categories as $category)
                        <option @if (old('category') == $category) selected @elseif ($game->category==$category) selected @endif value="{{ $category }}">{{ $category }}</option>
                    @endforeach
                </select>
                @error('category')
                    <div class=" invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-2">
                ...
                <input type="text" class="form-control form-control-sm @error('developer') is-invalid @enderror"
                    id="developer" aria-describedby="developer_help" name="developer"
            ...
            <div class="mb-2">
                ...
                <input type="text" class="form-control form-control-sm @error('publisher') is-invalid @enderror"
                    id="publisher" aria-describedby="publisher_help" name="publisher"
            ...
            <div class="mb-2">
                ...
                <input type="number" class="form-control form-control-sm @error('price') is-invalid @enderror" id="price"
                    aria-describedby="price_help" name="price" value="{{ old('price') ?? $game->price }}">
                @error('price')
                    <div class=" invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-check mb-3">
                <input class="form-check-input bg-dark" type="checkbox" id="for_adult" name="for_adult" value="true"
                    @if (old('for_adult') == 'true') checked  @elseif (old('for_adult') == '') @elseif ($game->for_adult == 1) checked @endif>
